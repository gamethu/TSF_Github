...
from keras.layers            import Dense, Dropout, Bidirectional, LSTM, Input
...
# Chuẩn bị dữ liệu: chỉ sử dụng cột Price
...
# Xây dựng mô hình Bidirectional LSTM với Input layer
model = Sequential()
...
# Định nghĩa callback để lưu checkpoint với phần mở rộng .keras
checkpointer = ModelCheckpoint(filepath       = "logs/bidirectioanl-lstm/checkpoint-{epoch:02d}.keras",
                               verbose        = 1,
                               save_best_only = True,
                               monitor        = 'val_loss',
                               mode           = 'min')
...
# Lưu mô hình
model.save("logs/bidirectioanl-lstm/lstm1layer_model.keras")
...
plt.title('Bi-LSTM -- So sánh giá dự báo và giá thực tế')
plt.xlabel('Thời gian')
plt.ylabel('Giá đóng cửa (USD)')
plt.legend()
plt.savefig('price_comparison_bidirectional_lstm.png')
plt.close()